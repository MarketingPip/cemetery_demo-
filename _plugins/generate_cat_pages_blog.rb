...
module Jekyll
  class GenerateTagPages < Generator
    ...
    def generate(site)
      ...
      all_tags = site.posts.docs.flat_map { |p| p.data['category'] || [] }.uniq
      Jekyll.logger.info "Found tags: #{all_tags.join(', ')}"

      all_tags.each do |tag|
        tagged_posts = site.posts.docs.select { |p| (p.data['category'] || []).include?(tag) }
    ...
    def paginate(site, tag, tagged_posts, page_num, total_pages)
      ...
      dir = page_num == 1 ? "blog/category/#{tag}" : "blog/category/#{tag}/page#{page_num}"
      name = "index.html"
