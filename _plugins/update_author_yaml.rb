  ...
  class UpdateAuthorYAML < Generator
    ...
    def generate(site)
      ...
      (site.collections['posts']&.docs || site.posts.docs).each do |post|
        ...
        formatted_date = safe_date_format(post.data['date'], '%Y/%m/%d')
        slug = post.data['slug'] || File.basename(post.relative_path, '.md').sub(/^\d{4}-\d{2}-\d{2}-/, '')
        date_str = safe_date_format(post.data['date'], '%Y-%m-%d')

        post_info = {
          'title'   => post.data['title'],
          'url'     => post.url || (formatted_date ? "/blog/#{formatted_date}/#{slug}" : "/blog/#{slug}"),
          'date'    => date_str,
      ...
      if site.collections['exhibits']
        site.collections['exhibits'].docs.each do |exhibit|
          ...
          date_str = safe_date_format(exhibit.data['date'], '%Y-%m-%d')
    ...
    def safe_date_format(date, format)
