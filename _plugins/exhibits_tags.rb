...
module Jekyll
  class GenerateTagPages < Generator
    ...
    def generate(site)
      ...
      all_tags = site.exhibits.docs.flat_map { |p| p.data['tags'] || [] }.uniq
      Jekyll.logger.info "Found tags: #{all_tags.join(', ')}"

      all_tags.each do |tag|
        tagged_posts = site.exhibits.docs.select { |p| (p.data['tags'] || []).include?(tag) }
    ...
    def paginate(site, tag, tagged_posts, page_num, total_pages)
      ...
      dir = page_num == 1 ? "exhibits/tag/#{tag}" : "exhibits/tag/#{tag}/page#{page_num}"
      name = "index.html"
